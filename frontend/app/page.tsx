...
export default function Home() {
  ...
  return (
    <AuthGuard>
      <AppShell>
        <div className="border-b border-cream-border bg-cream-card px-4 md:px-6 py-3">
        ...
        <div className="flex-1 p-4 md:p-6">
          ...
          {isError && (
            <p className="text-danger text-sm">
              Failed to load documents for PO "{activePoNumber}". Has it been
              uploaded yet?
            </p>
          )}
          {!isLoading && !isError && (
            <div className="text-sm text-text-subtle">
              ...
              <p>
                PO: {counts.po} | Invoices: {counts.fulfillment} | GRNs:{" "}
                {counts.delivery}
              </p>
            </div>
          )}
